...
const assignDeliveryPerson = async (req, res) => {
  try {
    ...
    await notify(
      order.user._id,
      '🚴 Order Assigned!',
      `Your order has been assigned to ${deliveryPerson.name}. It will be delivered soon!`,
      'order',
      `/orders/${order._id}`,
      'local_shipping'
    );
    ...
    await notify(
      ...
      `You have a new delivery order #${order._id.toString().slice(-6).toUpperCase()} to ${order.address?.city}.`,
      'order',
      '/delivery/orders',
      'delivery_dining'
    );
...
const cancelOrder = async (req, res) => {
  try {
    ...
    await notify(
      order.user._id,
      '❌ Order Cancelled',
      `Your order #${order._id.toString().slice(-6).toUpperCase()} has been cancelled. Refund will be processed shortly.`,
      'order',
      '/orders',
      'cancel'
    );

...
const SEGMENT_COLORS = {
  never_ordered: 'bg-blue-100 text-blue-700',
  churned:       'bg-red-100 text-red-700',
  at_risk:       'bg-amber-100 text-amber-700',
  low_value:     'bg-slate-100 text-slate-600',
};

const StatCard = ({ icon, label, value, color = 'text-primary' }) => (
  <div className="flex items-center gap-3 p-4 bg-white border dark:bg-slate-900 rounded-2xl border-slate-100 dark:border-slate-800">
...
export default function Algorithms() {
  ...
  useEffect(() => {
    if (tab === 'forecast' && !forecast)   load('forecast', () => API('/algo/inventory-forecast')).then(setForecast);
    if (tab === 'abandoned' && !abandoned) load('abandoned', () => API('/algo/abandoned-carts?hours=1')).then(setAbandoned);
    if (tab === 'coupons' && !coupons)     load('coupons', () => API('/algo/coupon-targets')).then(setCoupons);
  }, [tab]);

  const handleOptimizeRoutes = async () => {
    const data = await load('routes', () => API('/algo/optimize-routes', { method: 'POST' }));
    setRoutes(data);
  };

  const handleApplyRoutes = async () => {
    if (!routes?.assignments?.length) return;
    const data = await load('apply', () => API('/algo/optimize-routes/apply', {
      method: 'POST',
      body: JSON.stringify({ assignments: routes.assignments }),
    }));
    showToast(`✅ ${data.applied} orders auto-assigned!`);
    setRoutes(null);
  };

  const TABS = [
    { id: 'forecast',  icon: 'inventory',       label: 'Inventory Forecast' },
    { id: 'abandoned', icon: 'shopping_cart_off', label: 'Cart Abandonment' },
    { id: 'routes',    icon: 'route',            label: 'Route Optimizer' },
    { id: 'coupons',   icon: 'sell',             label: 'Coupon Targeting' },
  ];

  return (
    <div className="p-8 space-y-6">
      {toast && (
        <div className="fixed z-50 px-5 py-3 text-sm font-semibold text-white shadow-xl top-5 right-5 bg-slate-900 rounded-xl">{toast}</div>
      )}
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">AI-powered insights to grow your grocery business</p>
      </div>
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all
              ${tab === t.id ? 'bg-primary text-white shadow-md shadow-primary/25' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-primary/40'}`}>
            <span className="text-sm material-symbols-outlined">{t.icon}</span>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'forecast' && (
        <div className="space-y-4">
          {loading.forecast ? (
            <div className="flex items-center justify-center h-40"><span className="text-3xl material-symbols-outlined animate-spin text-primary">progress_activity</span></div>
          ) : forecast ? (
            <>
              <div className="grid grid-cols-3 gap-4">
                <StatCard icon="warning" label="Critical (≤3 days)" value={forecast.summary?.critical || 0} color="text-red-500" />
                <StatCard icon="error" label="High risk (≤7 days)" value={forecast.summary?.high || 0} color="text-orange-500" />
                <StatCard icon="info" label="Medium risk (≤14 days)" value={forecast.summary?.medium || 0} color="text-amber-500" />
              </div>
              <div className="overflow-hidden bg-white border dark:bg-slate-900 rounded-2xl border-slate-100 dark:border-slate-800">
                <table className="w-full text-sm">
                  <thead className="text-xs font-bold uppercase bg-slate-50 dark:bg-slate-800 text-slate-400">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {forecast.forecast?.map(f => (
                      <tr key={f.product._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-3">
                            <img src={f.product.image || ''} alt="" className="object-cover rounded-lg w-9 h-9 bg-slate-100" onError={e => { e.target.style.display='none'; }} />
                            <div>
                              <p className="font-semibold truncate max-w-[180px]">{f.product.name}</p>
                              <p className="text-xs text-slate-400">{f.product.category}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3 font-semibold text-right">{f.currentStock}</td>
                        <td className="px-4 py-3 text-right text-slate-500">{f.dailyVelocity}/day</td>
                        <td className="px-4 py-3 text-right">
                          <span className={`font-black ${f.daysUntilStockout <= 3 ? 'text-red-500' : f.daysUntilStockout <= 7 ? 'text-orange-500' : 'text-amber-500'}`}>
                            {f.daysUntilStockout === 999 ? '∞' : f.daysUntilStockout}
                          </span>
                        </td>
                        <td className="px-4 py-3 font-semibold text-right text-primary">+{f.suggestedReorder}</td>
                        <td className="px-4 py-3 text-center">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-bold capitalize ${RISK_COLORS[f.risk]}`}>{f.risk}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                {!forecast.forecast?.length && <p className="py-10 text-center text-slate-400">No at-risk products — inventory looks good!</p>}
              </div>
            </>
          ) : null}
        </div>
          ...
          {loading.abandoned ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-4">
                <StatCard icon="shopping_cart_off" label="Abandoned Carts" value={abandoned.count || 0} color="text-red-500" />
                <StatCard icon="currency_rupee" label="Est. Lost Revenue" value={`₹${(abandoned.carts || []).reduce((s, c) => s + c.cartValue, 0).toLocaleString()}`} color="text-amber-500" />
              </div>
              <div className="overflow-hidden bg-white border dark:bg-slate-900 rounded-2xl border-slate-100 dark:border-slate-800">
                <table className="w-full text-sm">
                  <thead className="text-xs font-bold uppercase bg-slate-50 dark:bg-slate-800 text-slate-400">
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {abandoned.carts?.map((c, i) => (
                      <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        <td className="px-5 py-3">
                          <p className="font-semibold">{c.userName}</p>
                          <p className="text-xs text-slate-400">{c.userEmail}</p>
                        </td>
                        <td className="px-4 py-3 font-black text-right text-primary">₹{c.cartValue}</td>
                        <td className="px-4 py-3 text-right text-slate-500">{c.itemCount}</td>
                        <td className="px-4 py-3 text-xs text-right text-slate-400">{c.hoursAgo}h ago</td>
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            {c.items?.slice(0, 3).map((item, j) => (
                              <img key={j} src={item.product?.image || ''} alt="" className="object-cover w-8 h-8 rounded bg-slate-100" onError={e => { e.target.style.display='none'; }} />
                            ))}
                          </div>
                        </td>
                      </tr>
                ...
                {!abandoned.carts?.length && <p className="py-10 text-center text-slate-400">No abandoned carts right now 🎉</p>}
              </div>
            </>
          ) : null}
        </div>
      ...
      {tab === 'routes' && (
        <div className="space-y-4">
          <div className="p-6 bg-white border dark:bg-slate-900 rounded-2xl border-slate-100 dark:border-slate-800">
            <h2 className="mb-2 font-bold">Delivery Route Optimizer</h2>
            <p className="mb-4 text-sm text-slate-500">Automatically assigns pending orders to the nearest available delivery person using Haversine distance algorithm.</p>
            <div className="flex gap-3">
              <button onClick={handleOptimizeRoutes} disabled={loading.routes}
                className="bg-primary text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-primary/90 disabled:opacity-60 flex items-center gap-2 shadow-lg shadow-primary/25">
                {loading.routes ? <span className="text-sm material-symbols-outlined animate-spin">progress_activity</span> : <span className="text-sm material-symbols-outlined">route</span>}
                Calculate Optimal Routes
              </button>
              {routes?.assignments?.length > 0 && (
                <button onClick={handleApplyRoutes} disabled={loading.apply}
                  className="bg-emerald-500 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-emerald-600 disabled:opacity-60 flex items-center gap-2">
                  {loading.apply ? <span className="text-sm material-symbols-outlined animate-spin">progress_activity</span> : <span className="text-sm material-symbols-outlined">check_circle</span>}
          ...
          {routes && (
            <>
              <div className="grid grid-cols-2 gap-4">
                <StatCard icon="pending_actions" label="Pending Orders" value={routes.totalOrders} color="text-amber-500" />
                <StatCard icon="person_pin" label="Available Riders" value={routes.totalPersons} color="text-primary" />
              </div>
              {routes.assignments?.length > 0 ? (
                <div className="overflow-hidden bg-white border dark:bg-slate-900 rounded-2xl border-slate-100 dark:border-slate-800">
                  <table className="w-full text-sm">
                    <thead className="text-xs font-bold uppercase bg-slate-50 dark:bg-slate-800 text-slate-400">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left">Assigned Rider</th>
                        <th className="px-4 py-3 text-right">Distance</th>
                        <th className="px-4 py-3 text-right">Est. Time</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                      {routes.assignments.map((a, i) => (
                        <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                          <td className="px-5 py-3 font-mono text-xs text-slate-500">#{a.orderId?.toString().slice(-8).toUpperCase()}</td>
                          <td className="px-4 py-3">
                            <p className="font-semibold">{a.customerName}</p>
                            <p className="text-xs text-slate-400">{a.orderAddress?.city}</p>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <div className="flex items-center justify-center text-xs font-bold rounded-full w-7 h-7 bg-primary/10 text-primary">
                                {a.deliveryPerson?.name?.charAt(0)}
                              </div>
                              <span className="font-semibold">{a.deliveryPerson?.name}</span>
                            </div>
                          </td>
                          <td className="px-4 py-3 text-right text-slate-500">{a.estimatedDistanceKm} km</td>
                          <td className="px-4 py-3 font-semibold text-right text-primary">{a.estimatedMinutes} min</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="py-10 text-center bg-white border text-slate-400 dark:bg-slate-900 rounded-2xl border-slate-100 dark:border-slate-800">No pending orders to assign</p>
              )}
            </>
          )}
        </div>
          ...
          {loading.coupons ? (
            ...
            <>
              <div className="grid grid-cols-4 gap-4">
                <StatCard icon="person_add" label="New Users" value={coupons.summary?.never_ordered || 0} color="text-blue-500" />
                <StatCard icon="person_off" label="Churned" value={coupons.summary?.churned || 0} color="text-red-500" />
                <StatCard icon="warning" label="At Risk" value={coupons.summary?.at_risk || 0} color="text-amber-500" />
                <StatCard icon="arrow_upward" label="Upsell" value={coupons.summary?.low_value || 0} color="text-slate-500" />
              </div>
              <div className="flex items-start gap-3 p-4 text-sm border bg-primary/5 border-primary/15 rounded-2xl text-slate-600 dark:text-slate-400">
                <span className="flex-shrink-0 material-symbols-outlined text-primary">lightbulb</span>
                <div>
                  <span className="font-bold text-slate-800 dark:text-slate-200">Smart targeting saves coupon budget.</span> Loyal recent customers are excluded — they'll buy anyway. Focus coupons on at-risk and churned users where they drive the most impact.
                </div>
              </div>
              <div className="overflow-hidden bg-white border dark:bg-slate-900 rounded-2xl border-slate-100 dark:border-slate-800">
                <table className="w-full text-sm">
                  <thead className="text-xs font-bold uppercase bg-slate-50 dark:bg-slate-800 text-slate-400">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-center">Suggested Coupon</th>
                      <th className="px-4 py-3 text-center">Priority</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {coupons.targets?.slice(0, 50).map((t, i) => (
                      <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        <td className="px-5 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${SEGMENT_COLORS[t.segment]}`}>{t.label}</span>
                        </td>
                        <td className="px-4 py-3 text-slate-500 text-xs max-w-[200px] truncate">{t.reason}</td>
                        <td className="px-4 py-3 text-center">
                          <span className="px-3 py-1 text-xs font-black tracking-widest rounded-lg bg-primary/10 text-primary">{t.suggestedCode}</span>
                          {t.suggestedDiscount > 0 && <span className="ml-1 text-xs text-slate-400">({t.suggestedDiscount}% off)</span>}
                        </td>
                        <td className="px-4 py-3 text-center">
                ...
                {!coupons.targets?.length && <p className="py-10 text-center text-slate-400">All customers are healthy — no coupon intervention needed</p>}
              </div>
            </>
          ) : null}
        </div>

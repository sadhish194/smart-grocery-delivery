...
//   'Baby Care',
// ];
// const BLANK = { name: '', price: '', originalPrice: '', category: 'Fruits & Vegetables', description: '', stock: '', image: '', unit: 'kg', brand: '', isOrganic: false, isFeatured: false, discount: '' };
...
const BLANK = { name: '', price: '', originalPrice: '', category: 'Fruits & Vegetables', description: '', stock: '', image: '', images: ['', '', ''], unit: 'kg', brand: '', isOrganic: false, isFeatured: false, discount: '' };
...
const ManageProducts = () => {
  ...
  const openAdd = () => { setForm(BLANK); setEditId(null); setShowModal(true); };
  const openEdit = (p) => {
    const imgs = p.images?.length > 0 ? [...p.images, '', '', ''].slice(0, 3) : [p.image || '', '', ''];
    setForm({ name: p.name, price: p.price, originalPrice: p.originalPrice || '', category: p.category, description: p.description || '', stock: p.stock, image: p.image || '', images: imgs, unit: p.unit || 'kg', brand: p.brand || '', isOrganic: p.isOrganic || false, isFeatured: p.isFeatured || false, discount: p.discount || '' });
    setEditId(p._id); setShowModal(true);
  };
                ...
                <div className="space-y-2">
                  {form.images.map((url, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      {/* Preview thumbnail */}
                      <div className="flex-shrink-0 w-10 h-10 overflow-hidden border rounded-lg border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
                        {url.trim() ? (
                          <img src={url} alt="" className="object-cover w-full h-full"
                            onError={e => { e.target.style.display='none'; }} />
                        ) : (
                          <div className="flex items-center justify-center w-full h-full">
                            <span className="text-sm material-symbols-outlined text-slate-300">image</span>
                          </div>
                        )}
                      </div>
                      <input
                        ...
                        className="flex-1 px-3 py-2 text-sm border outline-none border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800 focus:ring-2 focus:ring-primary/30"
                      />
                    </div>
                  ))}
                </div>
                <p className="text-xs text-slate-400 mt-1.5">First image is the main product image shown in listings.</p>
              </div>
              <div>
                <label className="block mb-1 text-xs font-semibold text-slate-600 dark:text-slate-400">Description</label>
                <textarea rows={2} value={form.description} onChange={e => setForm(f => ({...f, description: e.target.value}))}
